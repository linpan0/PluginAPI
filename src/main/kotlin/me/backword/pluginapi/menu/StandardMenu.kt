...
open class StandardMenu(private val name: String, private val size: Int) : Menu {
  ...
  fun button(index: Int, itemStack: ItemStack, action: ((InventoryClickEvent) -> Unit)? = { it.isCancelled = true }) =
    buttons.set(index, Button(itemStack, action))

  fun button(
    x: Int,
    y: Int,
    itemStack: ItemStack,
    action: ((InventoryClickEvent) -> Unit)? = { it.isCancelled = true }
  ) = buttons.set(y * 9 + x, Button(itemStack, action))
