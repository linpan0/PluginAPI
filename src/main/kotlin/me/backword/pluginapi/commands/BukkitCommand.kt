    ...
    override fun execute(sender: CommandSender, label: String, args: Array<String>): Boolean {
        try {
            ...
            when {
                ...
                args.size > command.maxLength -> sender.sendMessage("§9Arguments> §cToo many arguments! §7Usage: ${command.usage.simple()}")
                isPlayer && args.size < command.minPlayerLength || !isPlayer && args.size < command.minConsoleLength -> sender.sendMessage("§9Arguments> §cNot enough arguments! §7Usage: ${command.usage.simple()}")
